 ...
 * 
 * 1. DIRECT (default): Use SQL_USER and SQL_PASSWORD directly
 *    - Set: SQL_AUTH_METHOD=direct (or omit)
 *    - Requires: SQL_USER, SQL_PASSWORD
 * 
 * 2. IAM: Use AWS IAM credentials to get temporary Redshift credentials
 *    - Set: SQL_AUTH_METHOD=iam
 *    - Requires: SQL_USER (Redshift database user), SQL_CLUSTER_ID (Redshift cluster identifier)
 *    - Optional: SQL_AWS_REGION, SQL_AWS_PROFILE
 *    - Uses your AWS credentials (env vars, profile, or IAM role)
 * 
 * 3. SECRETS_MANAGER: Fetch credentials from AWS Secrets Manager
 *    - Set: SQL_AUTH_METHOD=secrets_manager
 *    - Requires: SQL_SECRET_ID (secret name or ARN)
 *    - Optional: SQL_AWS_REGION, SQL_AWS_PROFILE
 *    - Secret should contain JSON with: username, password (and optionally host, port, database)
 */
...
async function getConnectionConfig(): Promise<{
  ...
  if (!user || !password) {
    throw new Error(
      `Missing credentials. For auth method '${authMethod}', ensure required variables are set.`
    );
  }
  ...
  try {
    switch (name) {
      ...
      case 'get_sample_data': {
        const table = (args as { table: string; limit?: number }).table;
      ...
      case 'get_schema_context': {
        const presetName = (args as { preset: string }).preset;
